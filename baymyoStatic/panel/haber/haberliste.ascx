<%@ Control Language="C#" AutoEventWireup="true" CodeFile="haberliste.ascx.cs" Inherits="panel_haber_haberliste" %>
    ...
    <asp:GridView ID="dataGrid1" runat="server" GridLines="None" CssClass="dataGridView"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="Resim">
                ...
                <ItemTemplate>
                    <img src="<%# Settings.ImagesPath+((!string.IsNullOrEmpty(Eval("ResimUrl").ToString()))? "haber/" + Eval("ResimUrl") : "admin-yok.png") %>"
                        style="width: 100%" />
                </ItemTemplate>
            </asp:TemplateField>
                ...
                <ItemTemplate>
                    <div class="vertical-img-set">
                        <a href='<%# Settings.CreateLink("makale",Eval("ID"),Eval("Baslik")) %>'>
                            <%# "<img class=\"toolTip left\" title='" + DataBinder.Eval(Container.DataItem, "Baslik") + " haberi görüntülemek için tıkla!' src=\"" + Settings.IconsPath + "admin/link.png\"/>"%></a>
                        <a href='<%# Settings.PanelPath +"?go=makale&mklid="+ Eval("ID")  %>'>
                            <%# "<img class=\"toolTip\" title='" + DataBinder.Eval(Container.DataItem, "Baslik") + " adlı haberi düzeltmek için tıkla!' src=\"" + Settings.IconsPath + "admin/edit.png\"/>"%></a>
                        <a href='<%# Settings.PanelPath +"?go=manset&mdl=makale&mcid="+ Eval("ID")  %>'>
                            <%# "<img class=\"toolTip\" title='" + DataBinder.Eval(Container.DataItem, "Baslik") + " adlı haberi manşete eklemek için tıkla!' src=\"" + Settings.IconsPath + "admin/manset.png\"/>"%></a>
